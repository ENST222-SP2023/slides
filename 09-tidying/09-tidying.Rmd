...
- `values_from`:column in long format containing what should be values in the new columns in the wide format
]
.pull-right-wide[
```{r}
purchases %>%
...
table3 %>%
  separate(rate, 
           ...
           sep = "/") #<<
```
*Note that cases and population are character values. Use `convert = TRUE` to try and convert into a better data type.*
...
*Note the default is to separate the two values with `_`. Use `sep = ""` to remove the separator.
...
*Now the separator has been removed but date is a character. `convert = TRUE` doesn't work with the `unite()` function, but we can wrangle further to coerce into a number.
...
## Use `complete` if you want to explicitly include all missing values
...
Output includes returns for all possible combinations of `year` and `qtr`
